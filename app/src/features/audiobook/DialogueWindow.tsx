...
function plain(text: string, keyPrefix: string) {
  return text.split(OTHER_MARKER).map((piece, i) => {
    ...
    return isMarker
      ? <span key={`${keyPrefix}-m${i}`} className="text-faint">{piece}</span>
...
export function DialogueWindow({
  ...
  if (!stop) {
    return (
      <div className="h-32 overflow-y-auto rounded border border-border bg-bg-primary/60 px-3 py-2 text-xs leading-relaxed text-faint">
  ...
  return (
    <div
      ref={scrollRef}
      aria-label="Dialogue"
      className="h-32 overflow-y-auto rounded border border-border bg-bg-primary/60 px-3 py-2 font-serif text-xs leading-relaxed text-faint"
    >
