...
- **Scene break suggestions.** A new "Suggest Breaks" button in the editor toolbar asks the AI to read the open chapter and recommend where `---` scene breaks would strengthen its pacing (POV shifts, time jumps, location changes, the end of an emotional beat). It returns 3-7 quote-anchored suggestions with a one-line rationale and a confidence rating, rendered in the Writing Companion chat. As with every AI feature, it only suggests: you read the recommendations and insert the breaks yourself, so nothing in your manuscript changes automatically.
- **Enhance mode in the Writing Companion.** A new "Enhance" toggle (next to Draft mode) turns the AI into a writer-directed rewrite partner: highlight a passage, type what you want changed (sharpen the mood, work in a description of the setting, make a character's reply reluctant, fix the pacing), and the AI rewrites *just that selection* to your direction. It automatically sends the paragraphs around your selection as grounding for continuity, and honors any attached context per the Canon/Reference toggle (below). An amount control governs how long the rewrite is, independent of your instruction: **Restate** (rework the wording at about the same length), **Default** (a richer pass, ~1.5x to 2.2x, auto-split into paragraphs), and **Expanded** (most immersive, ~2.2x to 4x). The length target adapts to your selection size and is capped so a large highlight can't balloon into an incoherent wall of text. Enhance is mutually exclusive with Draft mode, output appears in the chat for you to copy in yourself (the AI never edits your manuscript), and the prose keeps the approved `--` punctuation, never em dashes.
- **Canon / Reference toggle for attachments.** The attachment popup ("+ Add") now has a toggle, with a tutorial helptip, controlling how the AI treats everything you attach (character profiles, outline, locations). **Canon** (default) treats them as established truth and keeps your writing consistent with them; **Reference** treats them as helpful reference but lets your typed instructions take precedence, so the AI draws on the details that fit the moment instead of clinging to every trait. Applies across Draft, Enhance, and chat. When attachments are set to Reference, the context bar shows a small "· reference" marker so the stance is never hidden.
- **"New ask" in the Writing Companion.** After a result, a "New ask" button starts a fresh AI context (a clean slate) without erasing your transcript, and re-attaches your context chips to the new ask. This keeps an unrelated follow-up (for example, enhancing a different passage further down the chapter) from being contaminated by the previous exchange. The button highlights automatically when you move to a different selection. A divider marks each new ask in the transcript.
...
- **Outline and Profile editors no longer jump to the bottom of the page while typing.** In the Outline Planner and Profile Builder, placing the cursor and scrolling to a comfortable spot would get undone the instant you started typing: the view snapped to the bottom on every keystroke, worse in longer sections. The cause was the auto-growing text fields, which momentarily collapse each field to re-measure its height; for a long field that large collapse made the browser yank the scroll container to the bottom. The resize now preserves the scroll position (new shared `autoSizeTextarea` helper).
