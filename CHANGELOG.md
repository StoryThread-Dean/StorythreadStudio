...
- **Enhance mode in the Writing Companion.** A new "Enhance" toggle (next to Draft mode) turns the AI into a prose-enrichment partner: highlight a passage and the AI rewrites *just that selection* as richer, more vividly staged prose, keeping every event, fact, and outcome intact while adding sensory detail and small staging (it never invents new plot, named characters, or changed outcomes). It automatically sends the paragraphs around your selection as grounding so continuity holds, and a one-time nudge suggests attaching your chapter, outline, and character profiles as context for the best results. An amount control offers three levels: **Default** (follow your typed instructions), **Minimum** (add 1-4 sentences), and **Expanded** (3x-5x the selection). Enhance is mutually exclusive with Draft mode, output appears in the chat for you to copy in yourself (the AI never edits your manuscript), and drafted prose keeps the approved `--` punctuation.
...
- **Editing surfaces no longer jump to the bottom of the page while typing.** Across the Outline Planner, Profile Builder, chapter editor, and Style Guide, placing the cursor and scrolling to a comfortable spot would get undone the instant you started typing: the view snapped to the bottom of the document on every keystroke. Two distinct causes are fixed: (1) the structured editors (Outline, Profiles) auto-grow their text fields by momentarily collapsing each field to re-measure it, which made the browser yank the scroll container to the bottom; the resize now preserves the scroll position (new shared `autoSizeTextarea` helper). (2) The chapter/Style Guide editor wrapped CodeMirror in a second scroll container that fought CodeMirror's own cursor-tracking; it now uses a single scroll container.
