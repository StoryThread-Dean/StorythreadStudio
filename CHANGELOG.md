...
- **The Weave map and the story scrubber.** Your world drawn as a graph, with a chapter slider along the bottom. Drag it and Threads appear when they are introduced, connections light up when they become true, and a developing relationship stays one line whose label changes rather than three lines stacked on each other. A spoiler toggle hides anything the reader has not learned yet. The map carries its own searchable list of entries in the corner, grouped by kind, so finding one thing among two hundred does not mean hunting for a dot -- and a full list view is a click away for reading, keyboard use or a world too large to draw.
