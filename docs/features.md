...
- **Book Details** — a section header at the top that opens the Book Details popout (formerly "Project Settings" behind a gear icon, now the single home for everything book-level): Book title, **Series name** for a
  book in a series, Description, Genre, Tone, Theme, Setting, Word Count target, Chapter Count target, Point of View, Tense, Target Audience, plus content mode and the model picker. All story fields but the two targets flow into AI prompts as STORY CONTEXT; the targets live in the outline worksheet and drive the Writing Progress gauge.
  The series name lives in `series.json` one level up and is shared by every
  book in the series; before v2.0.3 it could only be set in the "Make this a
  series" checkbox when the first book was created, and never changed again.
  Renaming either the book or the series writes the metadata only -- neither
  moves a folder, since every book's `series_path` and the recents list point
  at those paths.
...
### Personality spine, story roles and other names

The profile header carries a **Personality (Enneagram)** field -- 9 types -- and it is a STORED field: the character remembers its type rather than the control snapping back to blank. Choosing a type writes no text on its own. **Pick what fits** then opens the type as its individual lines, grouped by what they want, what they dread, how they behave, how they talk, what pressure does to them, and a blank for the writer to fill in. Nothing is ticked on open; **Essentials** takes wants + dreads + speech and **Everything** takes all of them, and neither writes until Add is pressed. A line already in the character is greyed from the live editor buffer, so an unsaved insert greys at once and an undo un-greys it; a line the writer has since edited is offered again. What lands is a `[core]` trait block on Main and a LABELLED paragraph on Side, and a second insert extends the existing block rather than creating a twin. Changing the type never touches text already inserted. All canned text shipped in code -- writer-initiated, zero AI calls. Spec: `docs/character-spine-spec.md`.

**Story Role** is one control beside the Role field, not two. It APPENDS: a character can be a Merchant and a Red Herring and an Everyman at once, and picking one already present does nothing. Around sixty roles in nine groups ordered by what the writer is looking for (Leads, Opposition, Allies and companions, Family and household, Love and attachment, Trade and community, Mystery and misdirection, Power and institution, Structural and mythic), plus four adult groups LAST, each prefixed "Adult:" so they can be skipped unread. Every role explains what it does in a story, in a floating panel; the roles carrying archetype guidance offer an explicit button to insert it into Personality Traits. Picking a role writes the role name and nothing else.

**Also known as** sits beside Status on EVERY kind, not characters only. Every other word the entry answers to -- James is Jim, Ashfall is the burned city -- so all of them resolve to one profile when the Weave reads the manuscript. Buffer-edited and manually saved like the rest of the screen (Weaving's own alias route writes immediately, which is right for a walkthrough and wrong here). A word another entry already answers to is refused and names it. Nothing is generated. Spec: `docs/weave-spec.md`, appendix 2.

### Quick Build (side/background profiles)

Opens at the top of Side/Background character profiles. Pick a Story Role (it mixes lines that suit that role into every row below, about half of each deal, and choosing one re-rolls immediately; every role covers every row. Held back while a spicier tier is on, because the role lines are written for the ordinary pools, and the panel says so. Re-derived from the profile's Role field on reopen, matching any comma-separated part, so "Merchant, Villain" finds Shadow / Villain), then reroll curated sentence-length options per section — Physical, Mannerism, Voice, Want/Motivation, and Hidden/Foreshadowing (secrets, tells, and planted details built to pay off later) — and click any option to append it to the matching section as a new line.
...
- **Appearance** -- **Theme** (light / dark / custom), **Interface size**, **Editor text size**, **Line spacing** and **Paragraph spacing**. Five controls, and the split between the first two is the one worth understanding: **Interface size** scales the app *around* your work -- menus, sidebars, dialogs, labels -- across seven steps from 16px to 24px, enough for a 4K display; **Editor text size** sizes the words *in* your manuscript, outline, notes, summary editors and the Audiobook Converter's narration editor, in typography points from 9 to 24 with a Custom box, and can also be nudged while writing with `Ctrl +` / `Ctrl -` / `Ctrl 0`. They are separate because chrome shares its box with buttons, tables and cards and cannot be stretched freely, while prose in a wrapping editor has no layout to break. The editor default is 12 pt, which is exactly the 16px it has always rendered at, so an upgrade resizes nobody's book. The two spacing controls follow a word processor's model: line spacing is Single / 1.5 lines / Double / a custom multiple, measured against the font's own metrics and shown with its computed value (`1.5 lines [1.75]`); paragraph spacing is space before and after each paragraph in points, defaulting to 0 and 8. They are two different jobs and both are needed -- line spacing only opens up the lines *inside* a paragraph, so on its own it cannot put a gap between them. Markdown editing surfaces only; Reader Mode keeps its own picker for a deliberately different job. **Custom** is a third theme: it opens an editor listing all fifty-six role colours the app is built from, grouped by job, each with a hex box, a system-picker swatch and a colour wheel that drives whichever row is selected. It seeds from the theme in use, previews live, saves only on request, and shows each text colour's live contrast so a writer can see when a choice has become hard to read. Stored in `settings.json`, applied as inline custom properties rather than a third CSS block. The **Audiobook Converter has its own Dark / Light / Custom** under Audiobook Settings > Look and feel, independent of the writing side, so a dark editor beside a paper Converter is a valid combination; its light half is warm paper like the editor's, with deliberately stronger small text. See [`appearance-spec.md`](appearance-spec.md) for the full design, including the text-contrast contract every shipped theme is held to
- **Debug options**
