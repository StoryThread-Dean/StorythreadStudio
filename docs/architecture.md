...
| The dashboard's Recent Projects list | `~/.storythread/storythread.json` (+ a one-generation `.bak`) |

The recent-projects list is written the same way as `settings.json`, and for the same reason: a scratch `.tmp` sibling, the current file snapshotted to `.bak` only if it parses, then an atomic replace via `replace_atomic`. Reading it back tries the live file, then the `.bak`, and heals the live file from the backup when it recovers. If neither generation can be read the store **raises** rather than returning an empty list, and `GET /api/projects/recent` turns that into an HTTP error. That distinction is load-bearing: the dashboard renders an empty list as "No recent projects yet", so answering `[]` for an unreadable file told a writer with six books that they had none -- and because tracking a project reads, mutates and writes, that empty list was then saved over the top and the rest of their books were gone. A read never moves or rewrites anything; preserving unreadable bytes is the write path's job. Pinned by [`test_recent_projects.py`](../backend/tests/test_recent_projects.py).
