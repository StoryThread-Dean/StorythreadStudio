...
decisions taken after the 2026-08-11 audit (appendix 1) read all 806 lines against
...
The work these amendments scheduled is done: every in-scope task shipped in
v2.0.0 and v2.0.1, and what was deferred is in `docs/roadmap.md`.
 ...
 **AMENDED 2026-08-13 (R8.6), on the writer's ruling: THE PER-BOOK ROLE LEVEL IS
 ...
 the provider research (docs/local-model-spec.md, appendix) is effectively a pre-written
 design doc for this release.
...
# Appendix 1 -- how this spec came to be checked in

Folded in from `docs/weave-spec-gaps.md` on 2026-08-25, which was a 2026-08-11 gap list. Every gap in it has since shipped or been deferred with a roadmap entry, so the list itself is history and the commit record holds it. This part is not history: it is the reason this repository has a spec-discipline rule at all.
...
# Appendix 2 -- names an entry answers to

Folded in from `docs/alias-spec.md` on 2026-08-25. It was written as a standalone file and should not have been: aliases are Weave machinery (`codex_alias`, `POST /api/codex/alias`, `build_alias_map`), so they belong in the Weave's own spec. Section numbers are prefixed `A` to keep them distinct from the spec's.

**What this is.** A character is rarely called one thing. Gwendolyn Barksdale is *Gwendolyn* on her birth certificate, *Gwen* to everyone who knows her, and *Willow* on stage. All three are her. This is how the writer says so, and how the app keeps the three pointing at one profile.
...
### A3. Where aliases live

#### A3.1 Both dialects, like every other field
...
#### A3.2 Which kinds get the control
...
#### A3.3 `name`, `display_name` and aliases stay three things
...
they never chose, and the whole point of A5 is that wrong bindings are
invisible.

---

### A5. Asking whether it is still true (C)
...
#### A5.3 Staleness comes free, from the stop key
...
### A9. Build status, 2026-08-25

**A3 and A4 are built.** Aliases round-trip through both parsers, the
editor is on every kind in the Profile Builder header, and the contract test
pins that neither dialect can drop them.

**A5 is NOT built.** The stop, the stop key carrying the entry's name,
